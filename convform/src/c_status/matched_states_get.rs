...
impl<'a> StringMatchingPool<'a> {
    pub fn get_matched_states(self, flow: &'a Flow) -> MatchedStates<'a> {
        ...
        crate::cnd_dbg!(&recomposed_match_items);

        // add to csi.context - intent is there, replace with new adjacent states
        add_context_intents(&mut csi, &recomposed_match_items, flow);
        rm_context_intents(&mut csi, &recomposed_match_items);
...
fn add_context_intents(csi: &mut CStatusIn, recomposed_match_items: &[MatchItem], flow: &Flow) {
    // each intent name can be found in flow and has context - if there are any missing, add them
    recomposed_match_items.iter().for_each(|m| {
        let matched_intent = flow.intents.iter().find(|i| *i.0 == m.intent).unwrap().1;

        matched_intent
            .context_intents
            .iter()
            .for_each(|(intent, adjacent)| {
                csi.context.insert(
                    intent.to_string(),
                    (
                        adjacent.iter().map(|s| s.to_string()).collect(),
                        matched_intent.intent_name.to_string(),
                    ),
                );
            })
    })
...
fn rm_context_intents(csi: &mut CStatusIn, recomposed_match_items: &[MatchItem]) {
    // each matchitem has an intent name -> if the intent name is in csi.context, remove it
    recomposed_match_items.iter().for_each(|m| {
        csi.context.remove(m.intent);
    })
}
